...
class MailTMBot:
    def __init__(self):
        ...
        self.api_token = "3e28d5e1bacf447089dcddb855d38a6b"
        self.email = ""
        self.token = ""

    async def create_account(self):
        if not self.api_token:
            raise RuntimeError("TWENTY_TWO_DO_TOKEN belum diset di environment variable.")
        timeout = aiohttp.ClientTimeout(total=30)
        async with aiohttp.ClientSession() as session:
            async with session.post(f"{self.base_url}/token", json={"token": self.api_token}, timeout=timeout) as r:
                data = await r.json(content_type=None)
                if r.status < 200 or r.status >= 300 or not data.get("status"):
                    raise RuntimeError(f"22.do /token gagal ({r.status}): {data.get('msg', 'Unknown error')}")
                self.token = data.get("data", {}).get("Bearer", "")
                if not self.token:
                    raise RuntimeError("22.do tidak mengembalikan Bearer token.")

            headers = {"Authorization": f"Bearer {self.token}", "Accept": "application/json"}
            async with session.post(f"{self.base_url}/account", headers=headers, timeout=timeout) as r:
                data = await r.json(content_type=None)
                if r.status < 200 or r.status >= 300 or not data.get("status"):
                    raise RuntimeError(f"22.do /account gagal ({r.status}): {data.get('msg', 'Unknown error')}")
                self.email = data.get("data", {}).get("email", "").strip()
                if not self.email:
                    raise RuntimeError("22.do tidak mengembalikan alamat email.")
        logger.info(f"Akun 22.do dibuat: {self.email}")

    async def _get_messages(self, session):
        headers = {"Authorization": f"Bearer {self.token}", "Accept": "application/json", "Cache-Control": "no-cache"}
        async with session.post(
            ...
            data = await r.json(content_type=None)
            if r.status < 200 or r.status >= 300 or not data.get("status"):
                logger.warning(f"22.do /inbox gagal ({r.status}): {data.get('msg', 'Unknown error')}")
                return []
            return sorted(data.get("data", []) or [], key=lambda x: x.get("time", 0), reverse=True)

    async def _get_message(self, session, message_id):
        headers = {"Authorization": f"Bearer {self.token}", "Accept": "application/json"}
        async with session.post(
            f"{self.base_url}/inbox/message",
            headers=headers,
    ...
    async def fetch_otp(self, timeout=60):
        if not self.token or not self.email:
            raise RuntimeError("Akun 22.do belum dibuat.")
        start_time = asyncio.get_event_loop().time()
        seen = set()
        async with aiohttp.ClientSession() as session:
            while asyncio.get_event_loop().time() - start_time < timeout:
                try:
                    for msg in await self._get_messages(session):
                        msg_id = msg.get("messageId")
                        if not msg_id or msg_id in seen: continue
                        seen.add(msg_id)
                        detail = await self._get_message(session, msg_id)
                        if not detail: continue
                        content = self._content(detail)
                        match = re.search(r"(?:otp\s*code|kode\s*konfirmasi|otp)[:\s\-]*([A-Za-z0-9]{6})", content, re.IGNORECASE)
                        if match:
                            logger.info(f"OTP berhasil dibaca: {match.group(1)}")
                            return match.group(1).strip()
                        words = re.findall(r"\b[A-Z0-9]{6}\b", content)
                        for w in words:
                            if not any(x in w.lower() for x in ["emalupe", "mail", "http", "com", "co.id", "xlsmart"]):
                                return w
                except Exception as e:
                    logger.error(f"Error saat fetch OTP 22.do: {e}")
    ...
    async def fetch_xl_confirmation_email(self, timeout=60):
        ...
        seen = set()
        async with aiohttp.ClientSession() as session:
            logger.info("Menunggu email konfirmasi eSIM dari XL melalui 22.do...")
            while asyncio.get_event_loop().time() - start_time < timeout:
                try:
                    for msg in await self._get_messages(session):
                        msg_id = msg.get("messageId")
                        ...
                        detail = await self._get_message(session, msg_id)
                        if not detail: continue
                        combined_content = self._content(detail)
                        raw_text = detail.get("body", "") or ""
                        raw_html = detail.get("html", "") or ""
                        if isinstance(raw_text, list): raw_text = "\n".join(str(x) for x in raw_text)
                        if isinstance(raw_html, list): raw_html = "\n".join(str(x) for x in raw_html)
                        if not str(raw_text).strip() and str(raw_html).strip():
                            raw_text = re.sub(r"<[^<]+?>", "", str(raw_html))
                        combined_content = f"{detail.get('subject', '')}\n{raw_text}\n{raw_html}"
                    ...
                    logger.error(f"Error saat ekstrak detail email XL dari 22.do: {e}")
